...
var failSound = new Audio("./sounds/reset.wav");
...
UI.prototype.draw = function (ctx) { //draw ui
	...
	var timeWidth = 200;
	var timeFactor = (this.game.time / this.game.maxTime);
	...
	ctx.lineWidth = 14;
	ctx.beginPath();
	ctx.moveTo(this.game.liveCamera.x + 300, this.game.liveCamera.y + 480);
	ctx.lineTo(this.game.liveCamera.x + 300 + timeWidth * timeFactor, this.game.liveCamera.y + 480);
	ctx.stroke();
    ...
    if (this.game.player1.dead) {
        ...
        ctx.globalAlpha = 0.5;
        ctx.fillStyle = "black";
		ctx.fillRect(this.game.liveCamera.x, this.game.liveCamera.y, this.game.liveCamera.width, this.game.liveCamera.height);
        ctx.globalAlpha = 1.0;
        ctx.font = "100px Calibri";
        ctx.textAlign = "center"; 
        ctx.fillStyle = "black";
        ctx.fillText("You've failed",400 + this.game.liveCamera.x + 3,250 + this.game.liveCamera.y + 3);
        ctx.font = "30px Calibri";
		ctx.fillText("It's OK. So did the real Twitch. Press [R] to play again.",400 + this.game.liveCamera.x + 3,350 + this.game.liveCamera.y + 3);
		ctx.fillText("Final Score: " + this.game.score, 400 + this.game.liveCamera.x + 3,450 + this.game.liveCamera.y + 3);
        ctx.fillStyle = "white";
        ctx.font = "100px Calibri";
        ctx.fillText("You've failed",400 + this.game.liveCamera.x,250 + this.game.liveCamera.y);
        ctx.font = "30px Calibri";
		ctx.fillText("It's OK. So did the real Twitch. Press [R] to play again.",400 + this.game.liveCamera.x,350 + this.game.liveCamera.y);
		ctx.fillText("Final Score: " + this.game.score, 400 + this.game.liveCamera.x,450 + this.game.liveCamera.y);
        ctx.globalAlpha = 1.0;
...
function Character(game) {
	...
	this.idleAnimation = new Animation(ASSET_MANAGER.getAsset("./img/Character/char_idle.png"), 0, 0, 153, 153, 1, 1, true, false, 0, 0);
	this.okAnimation = new Animation(ASSET_MANAGER.getAsset("./img/Character/char_pass.png"), 0, 0, 153, 153, 1, 1, true, false, 0, 0);
	this.noAnimation = new Animation(ASSET_MANAGER.getAsset("./img/Character/char_no.png"), 0, 0, 153, 153, 1, 1, true, false, 0, 0);
    
    this.currentAnimation = this.idleAnimation;
    
    Entity.call(this, game, -2300, 200);
	...
	if (this.teleportToX != -1) {
		...
		if (gameStarted) {
			...
			if (this.game.currentPhase < 1) {
				startMusic.play();
				this.game.addEntity(new InfoBox(this.game, "Press UP to let pass, DOWN to ban."));
				this.game.step = 0;
				this.game.addEntity(this.game.buttonChallenge);
...
ASSET_MANAGER.queueDownload("./img/Character/char_pass.png");
ASSET_MANAGER.queueDownload("./img/Character/char_no.png");
